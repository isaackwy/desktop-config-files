...
-- function for displaying the calendar
function calendar(color, gsub_str)
     io.write(
        string.format(
            string.rep("   ", daystart-1) ..
            string.rep(" %2d", days), seq(1,days)
        ):gsub(string.rep(".",21),"%0\n")
         :gsub((gsub_str):format(currentday),
               (color):format(currentday)
          ) .. "\n"
    )
end

-- If the current day is on either 1, 2 or 3, then highlight only that day.
-- Otherwise, extraneous numbers become highlighted.
if (currentday == 1 or currentday == 2 or currentday == 3)
then
    calendar(conky_color2, " %2d")
else
    calendar(conky_color, "%2d")
end
